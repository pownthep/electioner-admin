...
export class RepsComponent implements OnInit {
  ...
  onDelete(id: string) {
    const repList = JSON.parse(localStorage["rep_data"]);
    const repListNew = repList.filter((rep) => rep["_id"] != id);
    localStorage["rep_data"] = JSON.stringify(repListNew);
    this.dataSource = new MatTableDataSource(repListNew);
    this.data.deleteRep(id).subscribe(
      ...
      (err) => {
        ...
        localStorage["rep_data"] = JSON.stringify(repList);
        this.dataSource = new MatTableDataSource(repList);
      }
    );
  }
  ...
  onEdit(id: string) {
    const repList = JSON.parse(localStorage["rep_data"]);
    const selectedRep = repList.filter((rep) => rep["_id"] === id);
  ...
  getDataSource() {
    return this.data.getReps().subscribe((data) => {
...
export class RepEditForm implements OnInit {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public rep: any,
  ...
  onSave(f: any) {
    this.repModel.fname =
      f.value.fnameCtrl == "" ? this.rep.fname : f.value.fnameCtrl;
    this.repModel.lname =
      f.value.lnameCtrl == "" ? this.rep.lname : f.value.lnameCtrl;
    this.repModel.dob = f.value.dobCtrl == "" ? this.rep.dob : f.value.dobCtrl;
    this.repModel.party =
      f.value.partyCtrl == "" ? this.rep.party : f.value.partyCtrl;
    this.repModel.district =
      f.value.districtCtrl == "" ? this.rep.district : f.value.districtCtrl;
